    ...
    try:
        ...
        for line in file.readlines():
            ...
            if line.find('using:') >= 0:
                ...
                if x[1].strip() == 'emu':
                    source = 1
                elif x[1].strip() == 'net' and x[1].strip() == 'newcamd' and x[1].strip() == 'CCcam-s2s' or x[1].strip() == 'gbox':
                    source = 2
            ...
            if line.find('from:') >= 0:
                if line.find('local') >= 0:
                    ...
                    addr = x[1].strip()
            line.find('local') >= 0
            if line.find('slot-1') >= 0:
                source = 4
                if HardwareInfo().get_device_name() == 'dm800se':
                    source = 5
                
            HardwareInfo().get_device_name() == 'dm800se'
            if line.find('slot-2') >= 0:
                source = 5
                if HardwareInfo().get_device_name() == 'dm800se':
                    source = 4
                
            HardwareInfo().get_device_name() == 'dm800se'
            if line.find('decode:') >= 0:
