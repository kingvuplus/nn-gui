...
class nemesisBluePanel(Screen):
	...
	nemesisVER = "nemesis-PROJECT"
	nemesisVERDATE = "22-07-2015"
	CVSDATE = "2.7Exp 22-07-2015"
	...
	def __init__(self, session):
		...
		self["key_1"] = Label(_("Nemesis News"))
	...
	def keyNumberGlobal(self, number):
		if number == 1:
			...
			self.session.openWithCallback(self.executedScript, nemesisConsole, cmd, _('Downloading Nemesis-PROJECT info...'))
		elif number == 2:
			self.session.open(ParentalControlSetup)
		elif number == 3:
			self.message = "EDG-Nemesis Version " +  self.nemesisVER + "\nBased on cvs from " +  self.CVSDATE + " (Fixed by REDOUANE)\n(c) " + self.nemesisVERDATE + " by EDG-Nemesis Group\n\nhttp://www.edg-nemesis.tv/"
	...
	def executedScript(self, *answer):
		self.session.open(nemesisShowPanel, "/tmp/info.txt" ,_('DB Info'))
